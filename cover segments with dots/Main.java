...
class Intercept implements Comparable<Intercept> {
    ...
    public int getRight_end() {return right_end;}
...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < number_of_intercepts; i++) {
            ...
            if (points[j] >= intercepts.get(i).left_end) {k=0;  break;}
            }
        if (k==1) {number_of_points++; points[number_of_points-1] = intercepts.get(i).right_end;}
        }                                                                      //надёжный шаг
        System.out.println(number_of_points);
        for (int i = 0; i < number_of_points; i++) {System.out.print(points[i] + " ");}
    }
}